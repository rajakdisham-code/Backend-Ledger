...
/** 
 * - Create a new transactin 
 * The 10 - STEP TRANSFER FLOW :
    * 1. Validate request
    * 2. validate idempotency key
    * 3. Check account status
    * 4. Derive sender balance from ledger
    * 5. Check transaction (PENDING)
...
async function createTransaction(req , res){
    ...
    /**
     * 1.5 Check if fromAccount and toAccount are different
     */
    ...
    */
    ...
    try{
        ...
        return res.status(400).json({
            message: "Transaction is pending due to some issue, pls try after sometime "
        })
    }
    /**
     * 10. Send email notification
...
async function createInitialFundsTransaction(req, res){
    ...
    // Find the system user (user document with systemUser: true)
    ...
    // Find the system user's account
    const fromUserAccount = await accountModel.findOne({
        user: systemUser._id
    })
    if(!fromUserAccount){
